...
export default class B2b {
    ...
    init() {
        this.initAdv();
        this.createOverlay();
        this.initQuantityDocument();
        this.initSelerperTopBtb();
    }
    initAdv() {
        const ele = document.createElement('div');
        ele.id = 'adv_tip';
        ele.className = 'adv_tip';
        if ($('.footer').length > 0) $('.footer').after(ele);
    }
    initSelerperTopBtb() {
        const roleId = sessionStorage.getItem('roleId');
        const companyId = sessionStorage.getItem('companyId');
        const companyName = sessionStorage.getItem('companyName');
        const sattus = (roleId === '3' && companyId && $('.container [end-masquerade]').length === 0);
        ...
        if (sattus) {
            ...
            dashboard.bindEndMasqueradeEvent();
        }
    }
    initB2bButton() {
        const roleId = sessionStorage.getItem('roleId');
        ...
        if (B2bUser) {
            addressBook.getAddressActionPermission().then(() => {
                this.initNavBtn(roleId, companyId);
            }).catch((error) => swal({
                text: error,
                type: 'error',
            }));
        ...
        if ($("[href='/trade-professional-application/']").length === 0) {
            // custom for tygis
            $('.navUser.container').prepend(`<li class="navUser-item">
            <a class="navUser-action" href="/trade-professional-application/">Trade Professional Application</a>
            </li>`);
        }
    }
    initNavBtn(roleId, companyId = sessionStorage.getItem('companyId')) {
        const companyStatus = sessionStorage.getItem('companyStatus');
        const isEnabledB2bAddressBook = sessionStorage.getItem('isEnabledB2bAddressBook');
        const url = window.location.href;
        const $addressBookElementUrl = $(".navBar-action[href='/account.php?action=address_book']");
        if (roleId === '3') {
            $('.navBar ul').prepend(this.getBtnHtml('/dashboard/', 'Dashboard'));

            // custom for tygis - add link to account dropdown menu
            $('#navPages-account-topbar').prepend(`<li class="dropdown-menu-item">
            <a href="/dashboard/">Dashboard</a>
        </li>`)
        }
        if (companyId && companyStatus === '1') {
            if ($(".navBar-action[href='/shopping-lists/']").length === 0 && !sessionStorage.getItem('quoteCompany')) {
                $('.navBar ul').append(this.getBtnHtml('/shopping-lists/', 'Shopping Lists'));
            ...
            if ($addressBookElementUrl.length > 0 && isEnabledB2bAddressBook === '1') {
                $addressBookElementUrl.attr('href', '/address-book/');
            }
            if (roleId !== '2') {
                if ($(".navUser-action[href='/quick-order-pad/']").length === 0) {
                    // custom for tygis
                    $('.navUser.container').prepend(this.quickOrderBtn);
                }
                if ($(".navBar-action[href='/user-management/']").length === 0) {
                    if (roleId !== '3' && roleId) {
                        $('.navBar ul').append(this.getBtnHtml('/user-management/', 'User Management'));
                        // custom for tygis - add link to account dropdown menu
                        $('#navPages-account-topbar').prepend(`<li class="dropdown-menu-item">
            ...
            if (isEnabledB2bAddressBook === '1' && url.includes('address_book')) {
                window.location.href = '/address-book/';
            }
        }
        if (companyStatus !== '1') {
            $("[href='/quick-order-pad/']").parent('.navUser-item').remove();
            $("[href='/shopping-lists/']").parent('.b2b-upercase').remove();
            $("[href='/user-management/']").parent('.b2b-upercase').remove();
            if ($(".navBar-action[href='/address-book/']").length > 0) {
                $(".navBar-action[href='/address-book/']").attr('href', '/account.php?action=address_book');
            }
        }
        $("[href='/wishlist.php']").parent('li.navBar-item').remove();
        $("[href='/wishlist.php']").remove();
        $('.navBar-item .navBar-action').each((index, item) => {
            const text = $(item).text();
            $(item).text(text.replace('undefined', '0'));
        });
        $('.b2b-upercase .navBar-item').each(item => {
    ...
    addQuiteBtn() {
        const B2bUser = isB2bUser();
        const noQuote = $(".navBar ul [href='/quote-list/']").length === 0;

        if (B2bUser && noQuote) {
            $('.navBar ul').append(this.getBtnHtml('/quote-list/', 'Quotes'));
            // custom for tygis - add link to account dropdown menu
            $('#navPages-account-topbar').prepend(`<li class="dropdown-menu-item">
            <a href="/quote-list/">Quotes</a>
        </li>`)
        }
    }
    createOverlay() {
        ...
        `;
        $('body').append(frag);
        this.$overlay = $('#b2b_loading_overlay');
    }
    initQuantityDocument() {
        const addCrtBtn = $('article .card-figcaption-body a[href *= "/cart.php"]');
        addCrtBtn.attr('no-advqty-card-addToCart', '');
    }

    getBtnHtml(url, name) {
        const pageUrl = document.location.href;
        const isActive = pageUrl.includes(url);
        return `
            <li class="b2b-upercase navBar-item ${isActive ? 'is-active' : ''}">
