...
export default class CreateQuote {
    ...
    initPage(context) {
        ...
        const quotePreviewData = sessionStorage.getItem('quotePreviewData');
        hideSelerBox();
        ...
        b2bAjax.getQuote(quoteId)
            .then(res => {
                ...
                this.companyId = res.data.companyInfo.companyId;
                if (!quotePreviewData) {
                    this.buildSessionData(res.data);
                }
                this.data = JSON.parse(quotePreviewData);
    ...
    buildSessionData(data) {
        const quoteInfor = data.quoteInfo;
        // don't use ...obj,lower version can't support
        const expiredAt = DateTime.getMonthDayYear(data.quoteInfo.expiredAt);
        const quotePreviewData = {
            quoteId: quoteInfor.quoteId,
            productList: quoteInfor.productList,
            referenceNumber: quoteInfor.referenceNumber,
            quoteTitle: quoteInfor.title,
            quoteDescription: quoteInfor.description,
            expiredAt,
            additionalInfo: quoteInfor.additionalInfo,
            addressInfo: quoteInfor.addressInfo,
            status: quoteInfor.status,
            money: quoteInfor.money,
        };
        sessionStorage.setItem('quotePreviewData', JSON.stringify(quotePreviewData));
    }
    ...
    bindEvents() {
        ...
        $('body').on('click', '#create_new_quote_button', (e) => {
            ...
            const totalCount = this.pureNumber;
            this.data.description = $('#quote_description').val();
            this.data.title = $('#quote_title').val();
            ...
            this.data.subtotal = totalCount.subtotal;
            this.data.discount = totalCount.discount;
            this.data.grandTotal = totalCount.grandTotal;
            ...
            urlHelper.redirect('/quote-detail?edit');
        });
        ...
        $('body').on('click', '#update_quote', () => {
            ...
            const totalCount = this.pureNumber;
            const data = $.extend({}, this.data, {
                title: $('#quote_title').val(),
                description: $('#quote_description').val(),
                expiredAt: $('#quote_date').val(),
                subtotal: totalCount.subtotal,
                discount: totalCount.discount,
                grandTotal: totalCount.grandTotal,
                ...
                status: quotePreviewData.status,
            });
    ...
    renderAddress(data) {
        ...
        let defaultAddress;
        if (hasPreivewData && JSON.parse(hasPreivewData).addressInfo) {
    ...
    initCompanySelect(list) {
        let sessionCompanyId = sessionStorage.getItem('companyId');
        if (!sessionCompanyId) {
        ...
        this.$companySelect.trigger('change');
        // 2.3 add adress
        this.initAdress();
    }
    ...
    get pureNumber() {
        ...
        return {
            ...
            discount: (quoteData.grandTotal - quoteData.subtotal),
        };
    }
